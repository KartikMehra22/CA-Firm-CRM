...
/**
 * admin/inquiries.php
 * Full inquiry list with search, status filter, and pagination.
 */
...
try {
    ...
    error_log('[CA-Firm CRM] Inquiries DB Error: ' . $e->getMessage());
    $rows = [];
    $total_rows = 0;
    $total_pages = 1;
}
...
<div class="card">
  <div class="card__header">
    ...
    <span style="font-size:.8rem;color:var(--muted);">
      Showing <?= count($rows) ?> of <?= $total_rows ?>
    </span>
  </div>
          ...
          <td>
            <div class="action-btns">
              <a href="/admin/edit_inquiry.php?id=<?= (int)$row['id'] ?>"
                 ...
                 title="Edit / View"
                 aria-label="Edit inquiry <?= (int)$row['id'] ?>">✏️</a>
              <a href="/admin/delete_inquiry.php?id=<?= (int)$row['id'] ?>"
                 class="btn-icon btn-icon--delete"
                 title="Delete"
                 aria-label="Delete inquiry <?= (int)$row['id'] ?>">🗑️</a>
            </div>
          </td>
        </tr>
        <?php endforeach; ?>
