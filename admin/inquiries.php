  ...
  <div class="card__body" style="padding:1rem 1.5rem;">
    <form method="GET" action="/admin/inquiries.php" id="filterForm">
      <div class="filters">
        ...
        <input type="text" name="search" id="searchInput" class="filter-input filter-input--search"
          placeholder="Search name, email, mobile…" value="<?= htmlspecialchars($search) ?>" autocomplete="off">

        <!-- Status dropdown auto-submits the form on change -->
        <select name="status" id="statusFilter" class="filter-input"
        ...
        <?php if ($search !== '' || $status !== ''): ?>
          <a href="/admin/inquiries.php" class="filter-btn filter-btn--ghost"
            style="text-decoration:none;text-align:center;">Clear</a>
        <?php endif; ?>
...
<div class="card">
  <div class="card__header">
    ...
    <div style="display:flex;align-items:center;gap:.75rem;flex-wrap:wrap;">
      ...
      <a href="/admin/export_csv.php<?= $search || $status ? '?' . http_build_query(array_filter(['search' => $search, 'status' => $status])) : '' ?>"
        class="filter-btn filter-btn--primary" style="text-decoration:none;display:flex;align-items:center;gap:.4rem;"
  ...
  <?php else: ?>
          ...
          <?php foreach ($rows as $row): ?>
            <tr>
              ...
              <td style="max-width:160px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"
                title="<?= htmlspecialchars($row['service']) ?>">
              ...
              <td>
                <div class="action-btns">
                  <a href="/admin/edit_inquiry.php?id=<?= (int) $row['id'] ?>" class="btn-icon btn-icon--edit" title="Edit"
                    aria-label="Edit inquiry <?= (int) $row['id'] ?>"><i data-lucide="edit"></i></a>
                  <a href="/admin/delete_inquiry.php?id=<?= (int) $row['id'] ?>" class="btn-icon btn-icon--delete"
                    title="Delete" aria-label="Delete inquiry <?= (int) $row['id'] ?>"><i data-lucide="trash-2"></i></a>
                </div>
              </td>
            </tr>
          <?php endforeach; ?>
    ...
    <?php if ($total_pages > 1): ?>
      <div
        style="display:flex;justify-content:center;align-items:center;gap:.5rem;padding:1.25rem 1.5rem;border-top:1px solid var(--grey-200);flex-wrap:wrap;">
        <?php if ($page > 1): ?>
          <a href="<?= build_qs(['page' => $page - 1]) ?>" class="filter-btn filter-btn--ghost"
            style="text-decoration:none;"><i data-lucide="arrow-left"></i> Prev</a>
        <?php endif; ?>

        <?php for ($p = max(1, $page - 2); $p <= min($total_pages, $page + 2); $p++): ?>
          <a href="<?= build_qs(['page' => $p]) ?>"
            class="filter-btn <?= $p === $page ? 'filter-btn--primary' : 'filter-btn--ghost' ?>"
            style="text-decoration:none;min-width:36px;text-align:center;" <?= $p === $page ? 'aria-current="page"' : '' ?>>
            <?= $p ?>
          </a>
        <?php endfor; ?>
        ...
        <?php if ($page < $total_pages): ?>
          <a href="<?= build_qs(['page' => $page + 1]) ?>" class="filter-btn filter-btn--ghost"
            style="text-decoration:none;">Next <i data-lucide="arrow-right"></i></a>
        <?php endif; ?>
      </div>
    <?php endif; ?>
  <?php endif; ?>
